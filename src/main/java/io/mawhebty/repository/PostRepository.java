...
public interface PostRepository extends JpaRepository<Post, Long>, JpaSpecificationExecutor<Post> {
    ...
    // Count queries
    @Query(value = "SELECT COUNT(*) FROM posts WHERE owner_user_id = :ownerId", nativeQuery = true)
    Long countByOwnerId(@Param("ownerId") Long ownerId);

    @Query(value = "SELECT COUNT(*) FROM posts p " +
            "LEFT JOIN post_statuses ps ON p.status_id = ps.id " +
            "WHERE p.owner_user_id = :ownerId AND ps.name = 'PUBLISHED'", nativeQuery = true)
    Long countPublishedByOwnerId(@Param("ownerId") Long ownerId);

    // Find posts by IDs
    @Query(value = "SELECT * FROM posts WHERE id IN :ids", nativeQuery = true)
    List<Post> findByIds(@Param("ids") List<Long> ids);

    // Find post by ID and owner
    @Query(value = "SELECT * FROM posts WHERE id = :id AND owner_user_id = :ownerId", nativeQuery = true)
    Optional<Post> findByIdAndOwnerId(@Param("id") Long id, @Param("ownerId") Long ownerId);

    // Find post by ID and type
    @Query(value = "SELECT * FROM posts WHERE id = :id AND owner_user_id = :ownerId AND type_id = :typeId", nativeQuery = true)
    ...
    @Query("""
                ...
                WHERE p.ownerUser.id <> :ownerId
                  ...
                  AND (
                        ...
                        OR LOWER(p.caption) LIKE LOWER(CONCAT('%', :search, '%'))
                      )
            """)
    ...
    Optional<Post> findByIdAndStatusAndVisibility(Long id, PostStatus status, PostVisibility visibility);

    List<Post> findTop5ByCategoryAndIdNotAndStatusAndVisibilityOrderByCreatedAtDesc(
